...
export function computeHorizontalPosition(
  ...
  const altitude = asind(sind(observer.latitude) * sind(declination) + cosd(observer.latitude) * cosd(declination) * cosd(H));
  const azimuth = normalizeDegrees(atand2(sind(H), sind(observer.latitude) * cosd(H) - cosd(observer.latitude) * tand(declination)) + 180);
