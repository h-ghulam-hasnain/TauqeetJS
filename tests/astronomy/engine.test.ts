...
describe('TauqeetJS Technical Specification & Engine Validation', () => {
  ...
  describe('Years Limit', () => {
    it('should throw an error if the year is below -2000 or above 3000', () => {
      const pastDate = new Date();
      pastDate.setUTCFullYear(-2001);
      
      const futureDate = new Date();
      futureDate.setUTCFullYear(3001);

      const resultPast = getPrayerTimes({ lat, long, date: pastDate });
      const resultFuture = getPrayerTimes({ lat, long, date: futureDate });
      
      expect(resultPast.success).toBe(false);
      if (!resultPast.success) expect(resultPast.error).toMatch(/Year out of range/);
      
      expect(resultFuture.success).toBe(false);
      if (!resultFuture.success) expect(resultFuture.error).toMatch(/Year out of range/);
    });
  });
});
